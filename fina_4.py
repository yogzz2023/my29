...
def process_measurements(measurements, meas_tgt_pair, kalman_filter):
    ...
    for i, (r, az, el, mt) in enumerate(measurements):
        if i == 0:
            ...
            for tid, track in enumerate(meas_tgt_pair):
                ...
                # Check if measurement time is less than 50 milliseconds
                if track[4] - mt < 50:
                    if distance <= chi2_threshold:
                        group_associations.append(tid)
                        print(f"Measurement {i} grouped with track {tid} at time {mt}")
            ...
            print(f"Associated measurements at step {i}: {associated_measurements}")
...
time_list, r_list, az_list, el_list = process_measurements(measurements, meas_tgt_pair, kalman_filter)
