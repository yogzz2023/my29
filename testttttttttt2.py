    ...
    if i == 0:
        ...
        for tid, track in enumerate(meas_tgt_pair):
            ...
            if distance <= chi2_threshold:
                association_list.append(tid)

        print(f"Groups formed at step {i}: {association_list}")

        associations = {}
        for meas_id in association_list:
            meas = meas_tgt_pair[meas_id]
            predicted = np.dot(kalman_filter.H, kalman_filter.Sp)
            distance = mahalanobis_distance(predicted, np.array(meas[1:4]).reshape((3, 1)), cov_inv)
            associations[meas_id] = distance

        # print(f"Clusters formed at step {i}: {associations}")

        cluster = [(m, a) for m, a in associations.items()]
        hypotheses = []
        for meas_id in association_list:
            prob = 1 / len(association_list)
            hypotheses.append((meas_id, prob))
        ...
        if len(associated_measurements):
            z = np.zeros((3, 1))
            for (measurement_id, prob) in associated_measurements.items():
                meas = np.array([meas_tgt_pair[measurement_id][1], meas_tgt_pair[measurement_id][2], meas_tgt_pair[measurement_id][3]]).reshape((3, 1))
                z += prob * meas
            kalman_filter.Z = z
